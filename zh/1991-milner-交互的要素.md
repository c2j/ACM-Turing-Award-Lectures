...
> **图 5.** $\lambda$-演算归约 $(\lambda x.M[x])N \to M[N]$
> **图 6.** CCS 交互 $ax.P[x] \mid \bar{a}V.Q \to P[V] \mid Q$
> (此处应有图 5-6 的示意图)
...
项   M ::= x          变量
           ...
           M1(M2)     应用
(y 的出现是绑定的)
基本计算规则: (λy.M1[y])(M2) → M1[M2]
```

**图 10. $\pi$-演算的一种简单形式**
```
名称 x, y, z ...
动作项 A ::= \bar{x}z.P    沿 x 发送 z
              xy.P         沿 x 接收任何 y
项     P ::= A1 + ... + An 备选动作 (n ≥ 0)
              P1 | P2      组合
              νyP          限制
              !P           复制
(y 的出现是绑定的)
基本计算规则: xy.P1[y] | \bar{x}z.P2 → P1[z] | P2
```
...
> **图 7.** CCS 中的倍增进程 `Double \stackrel{def}{=} ax.\bar{b}(2 \times x). Double`
> **图 8.** $\pi$-演算交互 $ax.P[x] \mid \bar{a}v.Q \to P[v] \mid Q$
> **图 9.** $\lambda$-演算
> **图 10.** $\pi$-演算的一种简单形式
> **图 11.** $\pi$-演算中的移动电话网络
> (此处应有图 7-11 的示意图)
